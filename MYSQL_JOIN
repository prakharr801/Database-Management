...
select e.emp_id,e.emp_name,e.department_id,d.department_name from employees e inner join Departments d on e.department_id = d.department_id;

#2.	Write a query to retrieve all employees with their departments, even if the department is missing.
select e.emp_id,e.emp_name,d.department_name from employees e left join Departments d on e.department_id = d.department_id;

#3.	Write a query to get department details even if there are no employees in that department. 
select e.emp_id,e.emp_name,e.department_id,d.department_name from employees e right join Departments d on e.department_id = d.department_id;
...
select e.emp_id,e.emp_name,p.project_id from employees e left join project p on e.emp_id=p.emp_id where p.project_id is null;

#7.	Find departments with no employees using a RIGHT JOIN.
select d.department_id,d.department_name,e.emp_id from employees e right join Departments d on e.department_id = d.department_id where e.emp_id is null;
...
select e.emp_id,e.emp_name,e.department_id,d.department_name,p.project_id,p.project_name
from employees e inner join Departments d on e.department_id = d.department_id inner join project p on e.emp_id = p.emp_id where d.department_name = "it";

#10.Write a query to show employees and their project information (even if not assigned to a project).
select e.emp_id,e.emp_name,p.project_id from employees e left join project p on e.emp_id=p.emp_id ;

#11.Find employees who work in departments with names starting with 'A'.
select e.emp_id,e.emp_name,e.department_id,d.department_name from employees e inner join Departments d on e.department_id = d.department_id where department_name like"a%";

#12.Find the total number of employees in each department using GROUP BY and JOIN
select d.department_id, count(e.emp_id) as total_employees from employees e inner join Departments d on e.department_id = d.department_id group by d.department_id;
...
Departments d on e.department_id = d.department_id group by d.department_id having count(e.emp_id)>1;
